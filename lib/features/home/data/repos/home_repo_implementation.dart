...
class HomeRepoImpl extends HomeRepo {
  ...
  Future<Either<Failure, List<MoviesModel>>> getTopRated(int page) async {
    try {
      var data = await _apiService.get(endPoints: 'movie/top_rated', page: page);
  ...
  @override
  Future<Either<Failure, List<MoviesModel>>> searchMovies({required String movieName}) async{
   try {
  ...
  Future<Either<Failure, List<MoviesModel>>> getNowPlaying(int page) async{
    try {
      var data = await _apiService.get(endPoints: 'movie/now_playing', page: page);
      List<MoviesModel> movies = [];
      for (var item in data['results']) {
        movies.add(MoviesModel.fromJson(item));
      }
